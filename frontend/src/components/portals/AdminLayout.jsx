...
import { 
  LayoutDashboard, Users, Package, FolderKanban, CalendarClock, Handshake,
  ShoppingCart, FileQuestion, FileText, Factory, MessageSquare, Settings,
...
const navItems = [
  ...
  { path: '/admin/users', icon: Users, label: 'User Management', badge: 'Admin' },
  { path: '/admin/inventory', icon: Package, label: 'Plant Inventory' },
  { path: '/admin/projects', icon: FolderKanban, label: 'Projects' },
  { path: '/admin/amc', icon: CalendarClock, label: 'AMC Subscriptions' },
  { path: '/admin/partners', icon: Handshake, label: 'Partners' },
  { path: '/admin/orders', icon: ShoppingCart, label: 'Orders' },
  { path: '/admin/rfq', icon: FileQuestion, label: 'Bulk RFQ' },
                ...
                >
                  {({ isActive }) => (
                    <>
                      ...
                      {item.badge && <Badge variant="outline" className={`text-xs ${isActive ? 'border-white/30 text-white' : 'border-primary/20'}`}>{item.badge}</Badge>}
          ...
          <div className="p-4 border-t border-primary/10">
            <div className="flex items-center gap-3 mb-4 p-3 bg-surface/50 rounded-lg">
              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                <span className="text-primary font-bold">{user?.full_name?.charAt(0) || 'A'}</span>
              </div>
